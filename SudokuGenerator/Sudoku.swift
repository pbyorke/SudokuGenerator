...
//  SudokuGenerator
//
//  Created by Peter Yorke on 4/28/23.
//
...
// /************************************************************************************************************************************************************/
 ...
 class AlgorithmXSolver {
     ...
     private var N: Int // have to get this from the caller
     private var SIZE: Int // have to get this from the caller
     private var Grid: [[Int]]
     private var root: Node? = nil // this is the starting node of the linked list
     // a raw Array List for dynamically storing the solutions. It slows things
...
// MARK: - data structures
     class Node: Equatable {
         // we define a node that knows about four other nodes, as well as its column head (I folded ColumnNode into Node)
         var left:  Node?
         var right: Node?
         var up:    Node?
         var down:  Node?
     ...
     class ColumnID: Equatable { // helps us store information about that column
         var constraint = -1
         var number = -1
         var position = -1
         
         static func == (lhs: ColumnID, rhs: ColumnID) -> Bool {
     ...
     private func createDoubleLinkedLists(_ matrix: [[Int]]) -> Node? {
         root = Node() // the root is used as an entry-way to the linked list i.e. we access the list through the root
         ...
         for row in 0..<matrix.count {
             ...
             curColumn = root?.right
             var lastCreatedElement: Node? = nil
             var firstElement: Node? = nil
             for col in 0..<matrix[row].count {
                 if matrix[row][col] == 1 { // i.e. if the sparse matrix element has a 1 i.e. there is a clue here i.e. we were given this value in the Grid
                     ...
                     var colElement = curColumn
                     while colElement?.down != nil {
                         colElement = colElement?.down
                     }
                     colElement?.down = Node()
                     if firstElement == nil {
     ...
     private func search(_ index: Int) {
         ...
         while r != c {
             if index < solution.count {
                 solution.remove(at: index) // if we had to enter this loop again
             }
             if let r {
                 solution.insert(r, at: index) // the solution is added .add(index, element)
             }
             var j = r?.right
             while j != r {
                 cover(j?.head)
     ...
     private func mapSolvedToGrid() {
         var result = [Int](repeating: 0, count: N * N)
         // we use Iterators to iterate over every single element of the ArrayList
         ...
         for element in solution {
             var number = -1 // initialize number and cell number to be a value that can't occur
             var cellNo = -1
             var next = element
             while element != next {
                 if next.head?.info?.constraint == 0 { // if we're in the row constraint
                     number = next.head?.info?.number ?? 0
                 } else if next.head?.info?.constraint == 3 {
                     cellNo = next.head?.info?.position ?? 0
                 }
         ...
         var resultCounter = 0
         for r in 0..<N { // iterates for the rows {
             for c in 0..<N { // iterates for the columns {
                 Grid[r][c] = result[resultCounter]
                 resultCounter += 1
             }
         }
     } // end func mapSolvedToGrid
     ...
     private func choose() -> Node? {
         ...
         // That is what we do.
         var rightOfRoot = root?.right // we cast the node to the right of the root to be a ColumnNode
         var smallest = rightOfRoot
         while rightOfRoot?.right != root {
             rightOfRoot = rightOfRoot?.right
             if let lhs = rightOfRoot, let rhs = smallest { // choosing which column has the lowest size}
     ...
     private func cover(_ column: Node?) { // covers the column; used as a helper method for the search method. Pseudo code by Jonathan Chu (credited above)
     ...
     private func uncover(_ column: Node?) { // uncovers the column i.e. adds back all the nodes of the column to the linked list
